...
from backend.cover_lookup import find_best_cover
...
class FrameDisplayApp:
    ...
    async def _handle_spotify_track(self, track: TrackInfo) -> None:
        ...
        if cached is not None:
            ...
            # Upgrade Spotify's 640x640 cover via the cover-lookup chain:
            # iTunes (3000) -> MusicBrainz/CAA -> Deezer -> Spotify 640.
            if track.album:
                lookup_start = time.monotonic()
                best = await find_best_cover(
                    track.artist,
                    track.album,
                    spotify_url=track.cover_url,
                    resolve_apple_cdn=self.recognizer.resolve_cover,
                )
                if best:
                    track.cover_url = best
                log.info(
                    "[%s] Cover lookup done (%.1fs)",
                    label, time.monotonic() - lookup_start,
                )
